local command_list = {}
local command_helplist = {}
command_count = 1
_G.command_handler = {}

cl:on('messageCreate',function(msg)
	local args = msg.content
	local argv = args:split(' ')

	if command_list[argv[1]] == nil then return end

	if command_list[argv[1]].is_admin == true then
	local alwd_g = {['admin'] = true, ['owner'] = true}
	local user_g = groups[msg.author.id]
	if alwd_g[user_g] == true then command_list[argv[1]].cb(msg, argv, args) else msg:reply(msg.author.mentionString..' необходимо иметь группу `admin` или `owner`.') end
	else
	command_list[argv[1]].cb(msg, argv, args)
	end
end)


local function register(command, help, help_args, is_admin, cb)
--	command_list[pref..command] = { 'cb' = cb, 'help' = help, 'command' = pref..сommand, }
	command_list[pref..command] = {}
	command_list[pref..command]['cb'] = cb
	command_list[pref..command]['help'] = help
	command_list[pref..command]['command'] = 'command'
	command_list[pref..command]['is_admin'] = is_admin
	command_helplist[command_count] = {}
	command_helplist[command_count]['command'] = pref..command
	command_helplist[command_count]['help'] = help
	command_helplist[command_count]['help_args'] = help_args
	command_helplist[command_count]['is_admin'] = is_admin
	command_count = command_count + 1
end

_G.command_handler['register'] = register

command_handler.register('help', 'Выводит это сообщение', nil, false, function (msg, argv, args)
	local table = {
		title = '**Бот в разработке**',
		description = '**[Ссылка на GitHub страницу](https://github.com/ProfessorBrain/natsuki)\n[Ссылка на приглашение бота](https://discordapp.com/oauth2/authorize?client_id=651411296050216975&scope=bot&permissions=8192)**',
		author = {
			icon_url = msg.author.avatarURL,
			name = msg.author.tag
		},
		footer = {
			icon_url = cl.user.avatarURL,
			text = cl.user.tag
		},
		color = 3586419,
		fields = {},
		timestamp = disc.Date():toISO('T', 'Z')
	}

for i = 1,#command_helplist do
	table.fields[i] = {}
	if command_helplist[i].help_args == nil then
	table.fields[i]['name'] = command_helplist[i].command 
	else
	table.fields[i]['name'] = string.format('%s %s', command_helplist[i].command, command_helplist[i].help_args)
	end
	table.fields[i]['value'] = command_helplist[i].help
end
	msg:reply { embed = table }
end)

