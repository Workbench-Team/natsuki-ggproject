...
--	account_set_link(userid, argv[2], argv[3])
	account_set_link(userid, 'steam64id', argv[2])
...
--	if argv[3] then
	if argv[2] then
		local id = string.gsub(argv[2], '@', '')
	        ...
	        id = string.gsub(id, '>', '')
	        id = string.gsub(id, '!', '')
		if not cl:getUser(id) then msg:reply('Необходимо написать id пользователя или упомянуть его') return end
	        local userid = cl:getUser(id).id
--		local result = account_get_link(userid, argv[2])
		local result = account_get_link(userid, 'steam64id')
		msg:reply(string.format('steam64id пользователя %s: %s', cl:getUser(userid).tag, result))
		return
	end

--	local result = account_get_link(msg.author.id, argv[2])
	local result = account_get_link(msg.author.id, 'steam64id')
