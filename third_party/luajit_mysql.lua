...
ffi.cdef[[
      ...
      MYSQLwrap_t * mysql_real_connect( MYSQLwrap_t *mysql, const char *host, const char *user, const char *passwd, const char *db, unsigned int port, const char *unix_socket, unsigned long clientflag);
      ...
      for i=1,nrows do
         ...
         for i=1,nfields do
            ...
            elseif fdef.type == ffi.C.MYSQL_TYPE_TIMESTAMP or fdef.type == ffi.C.MYSQL_TYPE_DATETIME or fdef.type == ffi.C.MYSQL_TYPE_DATE or fdef.type == ffi.C.MYSQL_TYPE_TIME then 
               ...
               local timeStruct = ffi.new("MYSQL_TIMEwrap_t")
               local datestr = ffi.string( row[i-1] )
               local llt,r
               if fdef.type == ffi.C.MYSQL_TYPE_TIME then
                  r = ffi.C.str_to_time( datestr, string.len(datestr), timeStruct, w )
                  llt = ffi.C.TIME_to_ulonglong( timeStruct )
               else
                  r = ffi.C.str_to_datetime( datestr, string.len(datestr), timeStruct, 0, w )
                  if fdef.type == ffi.C.MYSQL_TYPE_DATE then
                     llt = ffi.C.TIME_to_ulonglong_date( timeStruct )
                     llt = llt * 100 * 100 * 100
...
local mysql_connect =
   function( host, port, user, password, db )
      if port == nil then port = 3306 end
      local mysql = ffi.C.mysql_init(nil)
      local conn = ffi.C.mysql_real_connect( mysql, host, user, password, db, port, NULL, 0 )
      if conn == nil then
